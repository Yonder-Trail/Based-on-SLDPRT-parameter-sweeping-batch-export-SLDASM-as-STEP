...
def SLDASM_to_STEP_NewRebuild(Global_Parameter,Global_Parameter_list,SLDPRT_Filesname,SLDPRT_Dir,SLDASM_files_Dir,Output_STEP_Dir,Componentindex,Planindex):
    ...
    #获取当前进程信息
    def Get_Progcess_Information(start_time,Countindex,SUMs,Planindex):
        global SW_app
        Elapsed_Time=time.time()-start_time
        Speed=Elapsed_Time/Countindex
        Remaining_Sec=(SUMs-Countindex)*Speed
        ...
        Progress=(Countindex/SUMs)*100
        print(f"[OK] 当前计划 {Planindex} | 完成 {(Countindex)}/{SUMs} | 进度：{Progress:.3f}%    [Time] 已用时间：{Elapsed_str} | 剩余时间：{Remaining_Time} | 预计完成：{Eta_str}")
        if Countindex%50==0:
    ...
    def Rebuild_SLDASM_to_STEP(Global_Parameter,Global_Parameter_list,SLDPRT_Dir,SLDASM_Target_Files,Output_STEP_Incomplete_Dir,Planindex):
        ...
        #总数
        SUMs=math.prod([len(Parameter_Arr) for Parameter_Arr in All_Parameter_Combinations])*len(SLDASM_Target_Files)
        ...
        for _,Parameter_Combination in enumerate(itertools.product(*All_Parameter_Combinations)):
            ...
            for SLDASM_Target_File_Index,SLDASM_Target_File in enumerate(SLDASM_Target_Files):
                Output_STEP_Complete_Dir=Output_STEP_Incomplete_Dir[SLDASM_Target_File_Index]+f"{Countindex}.step"
                ...
                gc.collect()#回收内存
                Get_Progcess_Information(start_time,Countindex,SUMs,Planindex)
