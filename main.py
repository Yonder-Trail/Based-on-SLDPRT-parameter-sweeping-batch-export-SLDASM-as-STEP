...
#本项目本项目依托 SolidWorks COM API 与 Python 实现自动化建模功能，可通过修改零件全局变量驱动装配体参数化重建，并完成 STEP 文件的批量导出。
#逻辑：用户前提需要有带有全局参数的零件文件以及与其相关的装配体文件，配合关系不变。本代码首先改变零件文件全局变量，再重建零件文件，基于重建装配体文件，最后导出为STEP文件，整体为批量操作
import os
import gc
import math
import time
